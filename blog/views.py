...
def manage_updates(request):
    ...
    return render(request, 'blog/manage_updates.html', {'updates': updates})
...
def delete_update(request, update_id):
    if request.method == 'POST':
        try:
            ...
            messages.error(request, "Update does not exist.")
    return redirect(reverse('manage_updates'))

def send_update(request):
    if request.method == 'POST':
        message = request.POST.get('message')
        Update.objects.create(message=message)
    return render(request, 'send_update.html')



def contact_us_details(request):
    contacts = Contact.objects.all()
    return render(request, 'blog/contact_us_details.html', {'contacts': contacts})
...
def first(request):
    ...
    updates = Update.objects.all()  # Assuming Update is the model for updates
    context = {
        'updates': updates
    }
